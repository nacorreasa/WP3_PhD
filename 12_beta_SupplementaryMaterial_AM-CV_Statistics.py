...
########################################################################################
def load_ds(bd_in):
    """Load CPM dataset from multiple files"""
...
def calcular_maximos_anuales(ds, var_name='wsa100m'):
    """Calculate annual maxima from full time series"""
    ...
    for year in unique_years:
        ...
        times.append(np.datetime64(f'{year}-07-01'))
    
    ds_max_anual = xr.concat(max_vals, dim=pd.DatetimeIndex(times, name='time'))
    return ds_max_anual

def calcular_promedio_maximos_anuales(da_model):
    """Calculate average of annual maxima for each pixel"""
    return da_model.mean(dim='time')

def calcular_cv_maximos_anuales(da_model):
    """Calculate CV of annual maxima for each pixel"""
    avg = da_model.mean(dim='time')
    std = da_model.std(dim='time')
    cv  = std / avg
    return cv
...
# %%
#########################################################################################
###------LOAD AND COMPUTE ANNUAL MAXIMA AND CV(SAME AS FIG 3 OF THE MANSURIPT)--------###
#########################################################################################
print("STEP 1: Computing Annual Maxima and CV from full datasets")
...
print("Computing annual maxima for all pixels...")
am_eth  = calcular_maximos_anuales(ds_eth)
am_cnrm = calcular_maximos_anuales(ds_cnrm)
am_cmcc = calcular_maximos_anuales(ds_cmcc)

print("Computing average annual maxima...")
avg_am_eth  = calcular_promedio_maximos_anuales(am_eth)
avg_am_cnrm = calcular_promedio_maximos_anuales(am_cnrm)
avg_am_cmcc = calcular_promedio_maximos_anuales(am_cmcc)

print("Computing coefficient of variation...")
cv_eth  = calcular_cv_maximos_anuales(am_eth)
cv_cnrm = calcular_cv_maximos_anuales(am_cnrm)
cv_cmcc = calcular_cv_maximos_anuales(am_cmcc)

print(f"\nGlobal statistics:")
print(f"  ETH  - Avg AM: {float(avg_am_eth.mean()):.2f} m/s, CV: {float(cv_eth.mean()):.3f}")
print(f"  CNRM - Avg AM: {float(avg_am_cnrm.mean()):.2f} m/s, CV: {float(cv_cnrm.mean()):.3f}")
print(f"  CMCC - Avg AM: {float(avg_am_cmcc.mean()):.2f} m/s, CV: {float(cv_cmcc.mean()):.3f}")

# Verify hierarchy (CNRM should have highest CV)
if float(cv_cnrm.mean()) > float(cv_eth.mean()) and float(cv_cnrm.mean()) > float(cv_cmcc.mean()):
    print("\n✓ CV hierarchy is CORRECT (CNRM > ETH, CMCC)")
else:
    print("\n⚠ WARNING: CV hierarchy is unexpected")
    print(f"  Expected: CNRM > ETH ≈ CMCC")
    print(f"  Actual: CNRM={float(cv_cnrm.mean()):.3f}, ETH={float(cv_eth.mean()):.3f}, CMCC={float(cv_cmcc.mean()):.3f}")
...
# %%
########################################################################################
##----------------EXTRACT VALUES BY SPATIAL CATEGORY (SPATIAL MASKING)---------------##
########################################################################################
print("STEP 3: Extracting values by spatial category")

# Convert xarray data to numpy for easier indexing
avg_am_eth_np  = avg_am_eth.values
avg_am_cnrm_np = avg_am_cnrm.values
avg_am_cmcc_np = avg_am_cmcc.values

cv_eth_np  = cv_eth.values
cv_cnrm_np = cv_cnrm.values
cv_cmcc_np = cv_cmcc.values

# Verify shapes match
assert avg_am_eth_np.shape == band1.shape, "Shape mismatch between data and categories"
assert cv_eth_np.shape == band1.shape, "Shape mismatch between data and categories"
...
for cat in fl_cats:
    ...
    # Get labels
    climate_label    = get_climate_label(climate_cat)
    roughness_label  = get_roughness_label(roughness_cat)
    topography_label = get_slope_label(topography_cat)
    
    # Create mask for this category
    mask_cat = (band1 == cat)
    n_pixels = np.sum(mask_cat)
    ...
    am_eth_cat  = avg_am_eth_np[mask_cat]
    am_cnrm_cat = avg_am_cnrm_np[mask_cat]
    am_cmcc_cat = avg_am_cmcc_np[mask_cat]
    
    cv_eth_cat  = cv_eth_np[mask_cat]
    cv_cnrm_cat = cv_cnrm_np[mask_cat]
    cv_cmcc_cat = cv_cmcc_np[mask_cat]
    
    # Remove NaNs
    am_eth_cat  = am_eth_cat[~np.isnan(am_eth_cat)]
    am_cnrm_cat = am_cnrm_cat[~np.isnan(am_cnrm_cat)]
    am_cmcc_cat = am_cmcc_cat[~np.isnan(am_cmcc_cat)]
    
    cv_eth_cat  = cv_eth_cat[~np.isnan(cv_eth_cat)]
    cv_cnrm_cat = cv_cnrm_cat[~np.isnan(cv_cnrm_cat)]
    cv_cmcc_cat = cv_cmcc_cat[~np.isnan(cv_cmcc_cat)]
    
    print(f"  Valid pixels after removing NaNs: {len(am_eth_cat)}")
    
    # Add to data structures
    
    # Climate
    if climate_label is not None and climate_label in data_am['climate']:
        data_am['climate'][climate_label]['ETH'].extend(am_eth_cat.tolist())
        data_am['climate'][climate_label]['CNRM'].extend(am_cnrm_cat.tolist())
        data_am['climate'][climate_label]['CMCC'].extend(am_cmcc_cat.tolist())
        
        data_cv['climate'][climate_label]['ETH'].extend(cv_eth_cat.tolist())
        data_cv['climate'][climate_label]['CNRM'].extend(cv_cnrm_cat.tolist())
        data_cv['climate'][climate_label]['CMCC'].extend(cv_cmcc_cat.tolist())
    
    # Roughness
    if roughness_label is not None and roughness_label in data_am['roughness']:
        data_am['roughness'][roughness_label]['ETH'].extend(am_eth_cat.tolist())
        data_am['roughness'][roughness_label]['CNRM'].extend(am_cnrm_cat.tolist())
        data_am['roughness'][roughness_label]['CMCC'].extend(am_cmcc_cat.tolist())
        
        data_cv['roughness'][roughness_label]['ETH'].extend(cv_eth_cat.tolist())
        data_cv['roughness'][roughness_label]['CNRM'].extend(cv_cnrm_cat.tolist())
        data_cv['roughness'][roughness_label]['CMCC'].extend(cv_cmcc_cat.tolist())
    
    # Topography
    if topography_label is not None and topography_label in data_am['topography']:
        data_am['topography'][topography_label]['ETH'].extend(am_eth_cat.tolist())
        data_am['topography'][topography_label]['CNRM'].extend(am_cnrm_cat.tolist())
        data_am['topography'][topography_label]['CMCC'].extend(am_cmcc_cat.tolist())
        
        data_cv['topography'][topography_label]['ETH'].extend(cv_eth_cat.tolist())
        data_cv['topography'][topography_label]['CNRM'].extend(cv_cnrm_cat.tolist())
        data_cv['topography'][topography_label]['CMCC'].extend(cv_cmcc_cat.tolist())
    
    total_pixels_processed += len(am_eth_cat)
...
# %%
########################################################################################
##-------------------------PREPARING OUTPUTS FOR BOXPLOTS-----------------------------##
########################################################################################
print("STEP 4: Preparing data for boxplots")
print("="*80)
...
print(f"  AM Climate: {len(am_climate_df)} values")
print(f"  AM Roughness: {len(am_roughness_df)} values")
print(f"  AM Topography: {len(am_topography_df)} values")
print(f"  CV Climate: {len(cv_climate_df)} values")
print(f"  CV Roughness: {len(cv_roughness_df)} values")
print(f"  CV Topography: {len(cv_topography_df)} values")
...
print("STEP 5: Creating boxplots")
